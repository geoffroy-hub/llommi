...
// ✅ BUG FIX : resetBlogForm() silencieux pour éviter le message "Édition annulée"
//              après une mise à jour réussie
// ✅ AMÉLIORATION : validation taille image côté UI (avertissement préventif)
(function() {
    'use strict';
    ...
    window.editBlog = async function(blogId) {
        try {
            ...
            if (!cancelBtn) {
                ...
                document.getElementById('blogForm').appendChild(cancelBtn);
            }
    ...
    /**
     * ✅ BUG FIX : resetBlogForm() — réinitialise le formulaire SANS afficher de message
     * Remplace l'appel à cancelEditBlog() depuis handleBlogSubmit() pour éviter
     * le message parasite "Édition annulée" qui s'affichait par-dessus le message de succès.
     */
    ...
    /**
     * Annuler l'édition d'un blog (appelée uniquement via le bouton "Annuler")
     */
    window.cancelEditBlog = function() {
        resetBlogForm();
        showStatus('Édition annulée', 'info'); // message uniquement quand annulation volontaire
    };
    ...
    async function handleBlogSubmit(e) {
        ...
        try {
            ...
            if (imageFile) {
                ...
                if (existingBlog) imageUrl = existingBlog.image_url;
            }
            ...
            if (currentEditingBlog) {
                ...
                // ✅ BUG FIX : utiliser resetBlogForm() au lieu de cancelEditBlog()
                // pour ne pas afficher "Édition annulée" par-dessus le message de succès
                resetBlogForm();
            } else {
                // CRÉATION
     ...
     * ✅ AMÉLIORATION : avertissement si l'image dépasse 10 Mo
     */
    function handleBlogImagePreview(e) {
        ...
        if (file && file.type.startsWith('image/')) {
            // ✅ AMÉLIORATION : avertir si image trop lourde
            if (file.size > 10 * 1024 * 1024) {
                showStatus('⚠️ Image volumineuse (' + (file.size / 1024 / 1024).toFixed(1) + ' Mo). Privilégiez des images < 10 Mo pour de meilleures performances.', 'warning');
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        waitForSupabase(() => {
            ...
            if (blogImageInput) blogImageInput.addEventListener('change', handleBlogImagePreview);
