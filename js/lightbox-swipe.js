...
// ✅ BUG FIX : attribut data-lightbox-bound pour éviter l'accumulation des listeners au reload
(function () {
  'use strict';
  ...
  // ✅ BUG FIX : data-lightbox-bound évite d'ajouter plusieurs fois le même listener
  // lors des rechargements successifs via reload() (ex: après chargement Supabase)
  function attachMediaListeners() {
    mediaItems.forEach((item, index) => {
      ...
      if (item.type === 'video') {
        ...
        if (parent) {
          // ✅ BUG FIX : ne pas attacher si déjà fait
          if (parent.dataset.lightboxBound) return;
          parent.dataset.lightboxBound = 'true';
